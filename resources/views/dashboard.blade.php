    ...
    <x-app-layout>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden border border-stone rounded-[26px] p-8 mb-6">
                    ...
                    <div class="mt-6 flex gap-4">
                        <form method="POST" action="{{ route('logout') }}" class="inline">
                            @csrf
                            <button type="submit" class="bg-[#1e5e3a] hover:bg-[#154329] text-white px-6 py-2.5 rounded-[26px] font-semibold transition-all">Logout</button>
                        </form>
                    </div>
                </div>
            </div>
...
@else
    ...
    <head>
        ...
        <title>{{ config('app.name', 'Kelana') }} - Customer Dashboard</title>
    ...
    <body class="bg-warm-cream text-near-black font-sans antialiased min-h-screen flex flex-col" x-data="Object.assign({ scrolled: false, activeCategory: 'all' }, wishlistCartData())" @scroll.window="scrolled = (window.pageYOffset > 50)">
        ...
        <nav class="w-full fixed top-0 left-0 z-50 transition-all duration-300 border-b"
            ...
            <div class="max-w-[1400px] mx-auto px-6 flex justify-between items-center">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <!-- Chat/Support -->
                    <a href="#" @click.prevent="isChatOpen = !isChatOpen; if(isChatOpen) { $nextTick(() => scrollChatToBottom()) }" class="text-white hover:text-[#1e5e3a] p-2 transition-colors relative" aria-label="Chat Support">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M8.625 9.75a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375m-13.5 3.01c0 1.6 1.123 2.994 2.707 3.227 1.087.16 2.185.283 3.293.369V21l4.184-4.183a1.14 1.14 0 01.778-.332 48.294 48.294 0 005.83-.498c1.585-.233 2.708-1.626 2.708-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0012 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018z"/>
                        </svg>
                        <span x-show="chatMessages.some(m => m.sender_type === 'admin' && !m.is_read)" class="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-red-500 rounded-full" style="display: none;"></span>
                    </a>
        ...
        <section class="relative min-h-[90vh] flex flex-col items-center justify-center text-center px-6 py-32 bg-[#0f1a15] overflow-hidden">
            ...
            <div class="relative z-10 w-full max-w-4xl mx-auto mt-24">
                ...
                <p class="text-white/80 text-lg md:text-xl max-w-2xl mx-auto mb-12">
                    Welcome back, {{ Auth::guard('customer')->user()->name }}. Ready for your next adventure?
                </p>
            ...
            <div class="max-w-[1400px] mx-auto">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    @foreach($bestSellers as $card)
                        <a href="{{ route('paket.detail', $card['id']) }}" class="group bg-white rounded-[26px] overflow-hidden border border-stone block hover:scale-[1.02] transition-transform duration-300">
        ...
        <!-- 7. Kemungkinan Terjual Habis (Limited Capacity) -->
        <section class="w-full bg-[#f4f3ed] py-12 px-6">
            <div class="max-w-[1400px] mx-auto">
                <div class="mb-8">
        ...
        <!-- 9. Tempat ikonik yang harus Anda lihat -->
        ...
        <!-- 11. "About Kelana" Storytelling Section -->

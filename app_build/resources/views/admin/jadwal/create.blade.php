...
<div class="space-y-8">
    ...
    <div class="flex justify-between items-center border-b border-stone pb-6">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-near-black">Tambah Jadwal & Penugasan</h1>
            <p class="text-graphite text-sm mt-1">Buat jadwal keberangkatan baru dan tugaskan Trip Leader.</p>
        </div>
        ...
        <form action="{{ route('admin.jadwal.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col">
                    ...
                    <select name="id_leader" class="w-full p-4 rounded-[26px] bg-warm-cream/40 border border-stone focus:border-electric-lime focus:ring-1 focus:ring-electric-lime focus:bg-white transition duration-150 text-near-black font-semibold text-sm">
                        <option value="">-- Pilih Trip Leader --</option>
                        @foreach($leaders as $leader)
                            <option value="{{ $leader->id_leader }}" {{ old('id_leader') == $leader->id_leader ? 'selected' : '' }}>
                                {{ $leader->nama_leader }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col">
                    ...
                    <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai') }}" class="w-full p-4 rounded-[26px] bg-warm-cream/40 border border-stone focus:border-electric-lime focus:ring-1 focus:ring-electric-lime focus:bg-white transition duration-150 text-near-black font-semibold text-sm">
                    @error('tanggal_mulai')
                        <span class="text-red-600 font-bold ml-2 mt-2 inline-block text-xs">! {{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="flex flex-col">
                    ...
                    <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai') }}" class="w-full p-4 rounded-[26px] bg-warm-cream/40 border border-stone focus:border-electric-lime focus:ring-1 focus:ring-electric-lime focus:bg-white transition duration-150 text-near-black font-semibold text-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col">
                    ...
                    <input type="number" name="kuota" value="{{ old('kuota') }}" placeholder="Contoh: 20" class="w-full p-4 rounded-[26px] bg-warm-cream/40 border border-stone focus:border-electric-lime focus:ring-1 focus:ring-electric-lime focus:bg-white transition duration-150 text-near-black font-semibold text-sm">
                    @error('kuota')
                        <span class="text-red-600 font-bold ml-2 mt-2 inline-block text-xs">! {{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="flex flex-col">
                    ...
                    <select name="status_trip" class="w-full p-4 rounded-[26px] bg-warm-cream/40 border border-stone focus:border-electric-lime focus:ring-1 focus:ring-electric-lime focus:bg-white transition duration-150 text-near-black font-semibold text-sm">
                        <option value="Draft" {{ old('status_trip') == 'Draft' ? 'selected' : '' }}>Draft</option>
                        <option value="Open" {{ old('status_trip', 'Open') == 'Open' ? 'selected' : '' }}>Open</option>
                        <option value="Berjalan" {{ old('status_trip') == 'Berjalan' ? 'selected' : '' }}>Berjalan</option>
                        <option value="Selesai" {{ old('status_trip') == 'Selesai' ? 'selected' : '' }}>Selesai</option>
                        <option value="Batal" {{ old('status_trip') == 'Batal' ? 'selected' : '' }}>Batal</option>
                    </select>
                    @error('status_trip')
                        <span class="text-red-600 font-bold ml-2 mt-2 inline-block text-xs">! {{ $message }}</span>
                    @enderror

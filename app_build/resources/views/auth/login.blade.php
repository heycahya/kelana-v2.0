<x-guest-layout>
    <div class="animate-fade-in">
        ...
        <!-- Typography Header -->
        <div class="mb-8">
            <h1 class="text-2xl font-semibold text-near-black tracking-tight leading-tight mb-1">Welcome back</h1>
            <p class="text-graphite text-sm">Please enter your details to sign in.</p>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div class="flex items-center justify-between mb-6">
                <label for="remember_me" class="inline-flex items-center cursor-pointer select-none">
                    <input id="remember_me" type="checkbox" class="rounded border-stone text-near-black focus:ring-near-black w-3.5 h-3.5 transition">
                    <span class="ms-2 text-xs text-graphite font-semibold hover:text-near-black transition-colors">{{ __('Remember me') }}</span>
                </label>

                @if (Route::has('password.request'))
                    <a class="text-xs text-graphite hover:text-near-black transition-colors font-semibold" href="{{ route('password.request') }}">
                        {{ __('Forgot password?') }}
                    </a>
                @endif
            </div>
            ...
            <!-- Google Login (Soft-Filled Button with hover/scale animation) -->
            <a href="#" class="w-full inline-flex justify-center items-center px-6 py-3.5 bg-stone/50 border border-transparent rounded-[26px] text-sm font-semibold text-near-black hover:bg-near-black hover:text-white hover:scale-[1.01] transition-all duration-300 ease-in-out mt-3 mb-6 gap-2.5">
            ...
            <div class="text-center pt-4 border-t border-stone/30">
                <span class="text-xs text-graphite font-medium">Don't have an account?</span>
                <a class="text-xs text-near-black font-bold hover:underline focus:outline-none transition-colors ml-1" href="{{ route('register') }}">
                    Sign up
                </a>
            </div>
        </form>
